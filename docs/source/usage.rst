...
it may run with other modules but some functionality will break down.
...
using the default values. In the channels parameter there should be a dict for each frame type.
This contains the buffer_length. This will be used as the size for the circular buffer.
...
Please see config_template.conf for a more descriptive structure.
